...
- запуск сервера с VLM + сервиса который автоматически сканирует директорию `ocr_input` на наличие изображений или PDF, производит процесс распознавания текста и сохраняет результат в директорию `ocr_result` (подходит для локального запуска)
